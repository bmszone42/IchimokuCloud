...
hist_above, bins_above = np.histogram(macd_df[macd_df['MACD histogram' > threshold], bins=macd_df.index, range=(threshold, .20))
hist_below, bins_below = np.histogram(macd_df[macd_df['MACD histogram' <= threshold], bins=macd_df.index, range=(-.20, threshold))
